...
> GPT-4o 发货了一个大多数开放模型无法复制的产品：一个 agent，听到语音、看到摄像头所见、并在 250ms 内回话。开放生态在 2024 和 2025 年剩余时间竞相达到该产品形态。Qwen2.5-Omni (2025 年 3 月) 是参考开放设计：Thinker（大型文本生成 transformer）加 Talker（并行语音生成 transformer），由流式语音 token 连接。Mini-Omni 简化了它，Moshi 匹配了其延迟，GLM-4-Voice 将其扩展到中文。本课阅读 Thinker-Talker 架构和让流式实时对话工作的延迟预算。
