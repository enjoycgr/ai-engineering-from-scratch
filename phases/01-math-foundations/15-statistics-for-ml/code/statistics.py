...
if __name__ == "__main__":
    ...
    print(f"\n偏斜数据：{skewed}")
    print(f"Mean:   {mean(skewed):.2f}  (被异常值拉偏)")
    print(f"Median: {median(skewed):.2f}  (对异常值鲁棒)")
    ...
    print("特征 1 和 2 相关（构造如此）。")
    print("特征 3 独立。")

    print("\n" + "=" * 60)
    print("假设检验：单样本 t-test")
    print("=" * 60)
    ...
    print(f"  在 alpha=0.05 下显著: {result['p_value'] < 0.05}")

    print("\n" + "=" * 60)
    print("假设检验：双样本 t-test")
    print("=" * 60)
    ...
    print(f"模型 A 折数: {cv_a}")
    print(f"模型 B 折数: {cv_b}")
    ...
    print(f"  观察差值: {ab1['observed_diff']:.2f}")
    print(f"  p-value: {ab1['t_test']['p_value']:.4f}")
    print(f"  显著 (t-test): {ab1['significant_ttest']}")
    print(f"  Cohen's d: {ab1['cohens_d']:.4f} ({ab1['effect_interpretation']})")

    print("\n测试 2：真实效应 (true_effect = 5)")
    ab2 = ab_test_simulator(n_per_group=200, true_effect=5.0)
    print(f"  A 组均值: {ab2['group_a_mean']:.2f}")
    print(f"  B 组均值: {ab2['group_b_mean']:.2f}")
    print(f"  观察差值: {ab2['observed_diff']:.2f}")
    print(f"  p-value: {ab2['t_test']['p_value']:.4f}")
    print(f"  显著 (t-test): {ab2['significant_ttest']}")
    print(f"  Cohen's d: {ab2['cohens_d']:.4f} ({ab2['effect_interpretation']})")

    print("\n" + "=" * 60)
    print("多重比较问题")
    print("=" * 60)
    print("\n20 次检验，无真实效应（所有零假设为真）：")
    multi = run_multiple_ab_tests(n_tests=20, true_effect=0.0)
    print(f"  显著检验（未校正）: {multi['uncorrected_significant']}/20")
    print(f"  显著检验（Bonferroni）:  {multi['corrected_significant']}/20")
    print(f"  alpha=0.05 下期望 false positives: ~1")
    print(f"  Bonferroni 校正 alpha: {0.05/20:.4f}")
    ...
    print(f'\n教训：大 n 可使微不足道的效应变得"显著"。')
    print("始终检查 effect size，而非仅看 p-value。")

    print("\n" + "=" * 60)
    print("统计功效模拟")
    print("=" * 60)
    print("\n我们能多大概率检测到真实效应 (true_effect=3)？")
    n_sims = 200
    detected = 0
    ...
    print(f"  ({detected}/{n_sims} 次模拟检测到效应)")
    ...
    print(f"  ({detected_large}/{n_sims} 次模拟检测到效应)")
    print("  更大的样本量提供更多检测真实效应的功效。")
