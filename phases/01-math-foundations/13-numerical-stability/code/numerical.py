...
def demo_float_precision():
    ...
    print(f"  Float32 最小正数 (normal): ~{1.175e-38:.2e}")
    ...
    print(f"  Naive 求和:  {total_naive:.10f}  (误差: {abs(total_naive - true_value):.2e})")
...
def demo_catastrophic_cancellation():
    ...
    print(f"  Naive 误差:   {abs(var_naive - true_var):.2e}")
    ...
    for x in [1.0, 1e-300, 1e-323, 0.0]:
        try:
            if x == 0.0:
                ...
                print(f"  log(1e-323) = {result:.2f}  (最接近能表示的)")
            else:
                result = math.log(x)
...
def demo_cross_entropy():
    ...
    print(f"  Loss: {ce:.10f}  (接近零，模型正确且自信)")
    ...
    print(f"  Loss: {ce:.4f}  (非常大，模型自信但错误)")
    print()


def demo_sigmoid_stability():
    print("=" * 60)
    print("演示 7：稳定 Sigmoid")
    print("=" * 60)
...
def demo_gradient_checking():
    ...
    print(f"  错误 analytical: {wrong_grad}")
    print(f"  正确 numerical: {[f'{g:.4f}' for g in numerical_f3]}")
...
def demo_nan_inf():
    ...
    print(f"  max    = nan (与 nan 比较总是 False)")
...
def demo_mixed_precision():
    ...
    print(f"  {n_grads} 个范围 [1e-9, 1e-5] 的梯度")
    print(f"  无缩放时的零: {zeros_without_scaling}/{n_grads} "
          f"({zeros_without_scaling/n_grads*100:.1f}%)")
    print(f"  有缩放时 (x{scale:.0f}): {zeros_with_scaling}/{n_grads} "
          f"({zeros_with_scaling/n_grads*100:.1f}%)")
    print(f"  缩放+转换+反缩放后: {zeros_after_roundtrip}/{n_grads} "
        ...
        if math.isinf(simulate_float16(scaled)):
            ...
            if no_overflow_steps >= growth_interval:
                ...
                if step < 100 or step % 100 == 0:
                    print(f"  {step:>6d}  {scale_factor:>12.0f}  稳定 -> 翻倍")
    print(f"  最终缩放因子: {scale_factor:.0f}")
    print()
...
def demo_layer_norm():
    ...
    for layer in range(10):
        ...
        if layer % 2 == 0:
            print(f"  层 {layer:>2d}: 最大={max_val:>12.2f}  值={[f'{v:.2f}' for v in values[:3]]}...")
    ...
    for layer in range(10):
        ...
        max_val = max(abs(v) for v in values)
        if layer % 2 == 0:
            print(f"  层 {layer:>2d}: 最大={max_val:>6.4f}  值={[f'{v:.4f}' for v in values[:3]]}...")
    print()
...
def demo_common_bugs():
    ...
    print(f"  稳定 cross-entropy 处理此情况: {cross_entropy_stable(1, logits):.4f}")
    ...
    try:
        ...
        print("  Naive softmax: 溢出错误 (exp(800) 太大)")
    ...
    print(f"  Naive:   {var_naive:.6f}  (误差: {abs(var_naive - true_var):.2e})")
    ...
    print(f"  1/sqrt(方差 + 1e-5) = {safe:.2f} (加 epsilon 安全)")
    print()
...
def demo_format_comparison():
    ...
    print(f"""
  格式       位数  指数  尾数    ~位数    最大值           最适合
  -------    ----  ---  ------  -------  ----------       --------
  float64    64    11   52      15-16    1.8e308          CPU 训练，累积
