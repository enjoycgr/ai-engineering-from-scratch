"""音频分类基线：对 mean+var 池化 MFCC 的 k-NN。

合成 4 类数据集：{200, 400, 800, 1600} Hz 纯音加高斯噪声。
训练、测试、打印混淆矩阵。
...
def main():
    ...
    print(f"  特征维度: {len(X_train[0])}  (13 MFCC 的 mean+var)")
    ...
    print()
    print("要点:")
    print("  - 对 mean+var MFCC 池化的 k-NN 是一个 surprisingly strong 基线")
    print("  - 真实流程使用 BEATs / AST 微调 + SpecAugment + mixup")
