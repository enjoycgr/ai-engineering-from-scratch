...
**类型:** 构建
**语言:** Python
**先修知识:** 阶段 4 课程 06 (YOLO (You Only Look Once) Detection), 阶段 4 课程 08 (Mask R-CNN (掩码区域卷积神经网络)), 阶段 4 课程 24 (SAM 3)
**时间:** ~60 minutes

## 学习目标

- Distinguish tracking-by-detection from query-based tracking (跟踪) and name the algorithm families (SORT, DeepSORT, ByteTrack, BoT-SORT, SAM 2 memory tracker, SAM 3.1 Object Multiplex)
- Implement IoU (交并比) + Hungarian assignment 从零开始 for classic tracking-by-detection
- Explain SAM 2's memory bank and why it handles occlusion better than IoU-based association
- Read the three tracking (跟踪) metrics (MOTA, IDF1, HOTA) and pick which one matters for a given use case
...
Tracking is essential to every video-facing product: sports analytics, surveillance, autonomous driving, medical video analysis, wildlife monitoring, wordmark counting. The core building blocks are shared: a per-frame detector, a motion model (Kalman filter or something richer), an association step (Hungarian algorithm on IoU (交并比) / cosine / learned features), and a track lifecycle (birth, update, death).

2026 brought two new patterns: **SAM 2 memory-based tracking (跟踪)** (feature-memory instead of motion-model association) and **SAM 3.1 Object Multiplex** (shared memory for many instances of the same concept). 本课 walks the classical stack first, then the memory-based approach.
...
Every tracker 你将 encounter in 2026 is a variation on this loop. The differences:

- **SORT** (2016): Kalman filter + IoU (交并比) Hungarian. Simple, fast, no appearance model.
- **DeepSORT** (2017): SORT + a CNN-based appearance feature per track (ReID embedding (嵌入)). Handles crossings better.
...
Given a `M x N` cost matrix (tracks x detections), find the one-to-one assignment that minimises total cost. Cost is usually `1 - IoU (交并比)(track_bbox, detection_bbox)` or negative cosine similarity of appearance features. 运行time is O((M+N)^3); for M, N up to ~1000 it is fast enough in Python via `scipy.optimize.linear_sum_assignment`.

### ByteTrack's key idea

Standard trackers drop low-confidence detections (< 0.5). ByteTrack keeps them around as **second-stage candidates**: after matching tracks to high-confidence detections, unmatched tracks try to match low-confidence detections with a slightly looser IoU (交并比) threshold. Recovers short occlusions, ID switches near crowds.

### SAM 2 memory-based tracking (跟踪)

SAM 2 handles video by keeping a **memory bank** of per-instance spatio-temporal features. Given a prompt (click, box, text) on one frame, it encodes the instance into memory. On subsequent frames, the memory is cross-attended against the new frame's features, and the decoder (解码器) produces a mask (掩码) for the same instance in the new frame.
...
Cons:
- Slower than ByteTrack for many-object tracking (跟踪).
- Memory bank grows; limits the context window.

### SAM 3.1 Object Multiplex

Prior SAM 2 / SAM 3 tracking (跟踪) keeps a separate memory bank per instance. For 50 objects, 50 memory banks. Object Multiplex (March 2026) collapses them into one shared memory with **per-instance query tokens**. Cost scales sub-linearly in number of instances.

Multiplex is the new default for crowd tracking (跟踪) in 2026: concert crowds, warehouse workers, traffic intersections.
...
- **IDF1 (ID F1)** — harmonic mean of ID precision and recall (召回率). Focuses specifically on how well each ground-truth track keeps its ID over time. Better than MOTA for ID-switch-sensitive tasks.
...
Fixed constant-velocity Kalman omitted for brevity — we use a simple IoU (交并比) association here; in production the Kalman predict is essential. The `sort` Python package provides the full version.
...
60 lines. Takes per-frame detections, 返回 per-frame track IDs. Real systems add the Kalman predict, ByteTrack's second-stage re-match, and appearance features.
...
- SAM 2 / SAM 3.1 — memory-based tracking (跟踪) via `processor.track()`.
...
1. **(Easy)** 运行 the synthetic tracker above with 3, 10, and 30 objects. Report ID-switch count in each case. Identify where the simple IoU-only association starts to fail.
...
3. **(Hard)** Integrate SAM 2's memory-based tracker (via `transformers`) as an alternative tracker backend. 运行 both SimpleTracker and SAM 2 on a 30-second clip of a crowd and compare ID-switch counts, manually labelling ground-truth IDs for 5 salient people.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| Tracking-by-detection | "Detect then associate" | Per-frame detector + Hungarian assignment on IoU (交并比) / appearance |
...
| Object Multiplex | "SAM 3.1 shared memory" | Single shared memory with per-instance queries for fast many-object tracking (跟踪) |
| HOTA | "Modern tracking (跟踪) metric" | Decomposes into detection and association accuracy; community standard |
...
- [HOTA (Luiten et al., 2020)](https://arxiv.org/abs/2009.07736) — decomposed tracking (跟踪) metric
