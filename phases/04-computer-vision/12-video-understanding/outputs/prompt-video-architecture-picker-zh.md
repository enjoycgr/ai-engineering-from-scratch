...
2. `signal == appearance` -> **2D+pool** with **ResNet-50** (ImageNet-pretrained, battle-tested default for server-side inference (推理)).
...
6. `signal == both` and `compute_budget in [server_gpu, batch]` -> **时间Sformer** with divided attention (注意力).
...
- Never recommend full joint spatio-temporal attention (注意力); use divided or factorised.
