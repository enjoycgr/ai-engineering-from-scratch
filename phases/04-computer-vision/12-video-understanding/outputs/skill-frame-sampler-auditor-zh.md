...
description: Audit a video pipeline's frame sampler (采样器) for off-by-one, short-clip handling, and crop consistency
version: 1.0.0
phase: 4
lesson: 12
...
- Reproducing numbers from a paper and 训练 accuracy is lower than reported.
- Debugging a video model whose eval accuracy is unstable across runs.

## Inputs

- `sampler_code`: Python function that takes (num_frames_total, T) and 返回 T indices.
...
Feed `num_frames_total < T`. Every returned index must be in `[0, num_frames_total - 1]`. The standard padding (填充) policy is to repeat the last frame for the remaining positions.
...
### 5. Determinism
Call the sampler (采样器) twice with the same inputs and (for deterministic samplers) the same RNG. Indices should match.

### 6. Crop consistency
If the pipeline also 返回 a spatial crop per frame, run the sampler (采样器) twice for the same clip with the same seed and confirm every frame uses the same crop box (same `(x, y, w, h)`). Different crops per frame inside one clip destroys temporal coherence and is a classic silent bug. Acceptable variation: augmentation applied *per clip*, consistent within a clip.
...
- Never mark a sampler (采样器) "ok" if short-clip handling 返回 out-of-range indices.
- Dense samplers should never return a window that crosses `num_frames_total - 1`.
- If the sampler (采样器) is stochastic (dense), test determinism only with an explicit seeded RNG.
