...
- Implement frame sampling, temporal pooling (池化), and a 2D+pool baseline classifier in PyTorch
...
A 30-second video at 30 fps is 900 images. Naively, video classification (分类) is image (图像) classification run 900 times followed by some kind of aggregation. That works when the action is visible in almost every frame (sports, cooking, exercise videos) and fails badly when the action is defined by motion itself: "pushing something from left to right" looks like two still objects in every single frame.

The core 问题 for every video architecture is: when does temporal structure get modelled, and how? The answer drives everything else — compute cost, pre训练 strategy, whether you can reuse ImageNet weights, what datasets the model trains on.

本课 is deliberately shorter than the static-image lessons. The core image (图像) machinery is already in place, and video understanding is mostly about the temporal story: sampling, modelling, and aggregating.
...
Take a 2D CNN (卷积神经网络) (ResNet (残差网络), EfficientNet, ViT (视觉 Transformer)). 运行方式 independently on every sampled frame. Average (or max-pool, or attention-pool) the per-frame embeddings. Feed the pooled vector to a classifier.

Pros:
- ImageNet pre训练 transfers directly.
- Simplest to implement.
- Cheap: T frames * single-image inference (推理) cost.

Cons:
- Cannot model motion. Action = aggregate of appearances.
- Temporal pooling (池化) is order-invariant; "open door" and "close door" look the same.

When to use: appearance-heavy tasks, transfer learning (迁移学习) on small video datasets, initial baselines.
...
I3D trick: take a pretrained 2D ImageNet model, "inflate" each 2D kernel (卷积核) by copying it along a new time axis. A 3x3 2D conv becomes a 3x3x3 3D conv. This gives the 3D model strong pretrained weights instead of 训练 从零开始.

Pros:
- Directly models motion.
- I3D inflation gives free transfer learning (迁移学习).

Cons:
- T/8 more FLOPs than the 2D counterpart (for temporal kernel (卷积核) of 3 stacked 3 times).
...
Tokenise the video into a grid of space-time patches and attend across all of them. 时间Sformer, ViViT, Video Swin, VideoMAE.

Attention patterns that matter:
- **Joint** — one big attention (注意力) over (t, h, w). Quadratic in `T*H*W`; expensive.
- **Divided** — two attentions per block: one over time, one over space. Linear-ish scaling.
- **Factorised** — time attention (注意力) alternates with space attention across blocks.

Pros:
- SOTA accuracy on every major benchmark.
- Transfers from image (图像) transformers (ViT (视觉 Transformer)) via patch (图像块) inflation.
- Supports long-context video via sparse attention (注意力).

Cons:
- Compute-hungry.
- Requires careful attention (注意力) pattern choice or runtime balloons.
...
- **AVA** — action *localisation* in space and time; harder than classification (分类).

## 构建 It

### Step 1: Frame sampler (采样器)

Uniform and dense samplers that work on a list of frames (or a video tensor (张量)).
...
Both return `T` indices that you use to slice the video tensor (张量).

### Step 2: A 2D+pool baseline

运行 a 2D ResNet-18 over every frame, average-pool features, classify.
...
Eleven million parameters, ImageNet pretrained, runs per-frame, averages, 分类. This baseline is often within 5-10 points of proper 3D models on appearance-heavy tasks — sometimes better, because it reuses a stronger ImageNet backbone (骨干网络).
...
- `torchvision.models.video` — R(2+1)D, MViT, Swin3D with pretrained Kinetics weights. Same API as image (图像) models.
...
- `outputs/skill-frame-sampler-auditor.md` — a skill that inspects a video pipeline's sampler (采样器) and flags common bugs: off-by-one index, uneven sampling when `num_frames < T`, lack of aspect-preserving crop, etc.

## 练习

1. **(Easy)** Compute FLOPs (approximate) for FramePool with T=8 vs an I3D-style 3D ResNet (残差网络) with T=8. Justify why 2D+pool is 3-5x cheaper.
...
3. **(Hard)** 构建 an R(2+1)D-18 by replacing every Conv2d in a ResNet-18 with `Conv2Plus1D`. Inflate the first conv's weights from an ImageNet-pretrained ResNet-18. Train on the motion dataset from exercise 2 and beat FramePool.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| 2D + pool | "Per-frame classifier" | 运行 a 2D CNN (卷积神经网络) on every sampled frame, average-pool features across time, classify |
| 3D convolution (卷积) | "Spatio-temporal kernel (卷积核)" | Kernel that convolves over (T, H, W); can model motion natively |
...
| Divided attention (注意力) | "时间 then space" | Transformer block with two attentions per layer: one over tokens at the same frame, one over tokens at the same position |
...
| Kinetics | "The ImageNet of video" | 400-700 action classes, 300k+ YouTube clips, the standard video pre训练 corpus |
...
- [时间Sformer: Is Space-时间 Attention All You Need? (Bertasius et al., 2021)](https://arxiv.org/abs/2102.05095) — the first strong video transformer
- [VideoMAE (Tong et al., 2022)](https://arxiv.org/abs/2203.12602) — masked autoencoder pre训练 for video; current dominant pre训练 recipe
