...
description: Walk a CNN (卷积神经网络) spec layer by layer and report output shape, receptive field, and parameter count for every block
version: 1.0.0
phase: 4
lesson: 2
...
A deterministic helper for planning or debugging a CNN (卷积神经网络). Given an input shape and a list of layer specs, trace shapes, receptive fields, and parameter counts without running the model.

## When to use

- Designing a new CNN (卷积神经网络) and you want to verify every downsample lands on a clean size.
- Reading a paper and translating its architecture table into code.
- A pretrained backbone (骨干网络) crashes with a shape mismatch at the classifier head and you need to know which layer changed the spatial size.
- Comparing two backbones on parameter efficiency before 训练 either.
...
1. **Initialise trace** with `(C, H, W)`, receptive field `1`, effective stride (步幅) `1`, cumulative params `0`.
...
2. **For each layer**, update in this order:
   ...
   - Update receptive field and effective stride (步幅):
     ...
     - Flatten / linear: RF and effective stride (步幅) are no longer meaningful; freeze them to the values before the flatten and omit from subsequent rows.
   - Compute params:
...
3. **Detect problems** and flag them:
   - Non-integer output size (misaligned stride (步幅)/padding (填充)).
...
5. **总结 line**: final `(C, H, W)`, final receptive field, total params, warnings.
