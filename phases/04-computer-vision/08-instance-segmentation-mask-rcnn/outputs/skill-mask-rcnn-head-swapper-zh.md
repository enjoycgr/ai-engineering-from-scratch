...
description: Generate the exact code for swapping box and mask (掩码) heads on a torchvision Mask R-CNN (掩码区域卷积神经网络) for a custom num_classes
version: 1.0.0
phase: 4
lesson: 8
tags: [computer-vision, mask-rcnn, fine-tuning (微调), torchvision]
---

# Mask R-CNN (掩码区域卷积神经网络) Head Swapper

Produces the head-swap boilerplate for Mask R-CNN (掩码区域卷积神经网络) specifically. The template below assumes `model.roi_heads.box_predictor` and `model.roi_heads.mask_predictor`, which exist on `maskrcnn_resnet50_fpn` and `maskrcnn_resnet50_fpn_v2` only. Faster R-CNN has a box predictor but no mask (掩码) predictor; RetinaNet uses `RetinaNetHead` and has no `roi_heads` at all — both require different skills.

## When to use

- Fine-tuning `maskrcnn_resnet50_fpn` or `maskrcnn_resnet50_fpn_v2` on a custom class set.
- Porting a Mask R-CNN (掩码区域卷积神经网络) checkpoint trained on COCO to a non-COCO class count.
- Debugging a Mask R-CNN (掩码区域卷积神经网络) 训练 run that crashes on `cls_score.out_features` or `mask_predictor` mismatch.
...
- `freeze`: one of `backbone (骨干网络)`, `backbone_fpn`, `none`.
...
- `backbone (骨干网络)` ->
  ```python
  for p in model.backbone (骨干网络).parameters():
...
- `backbone_fpn` ->
  ...
  for p in model.backbone (骨干网络).parameters():
      p.requires_grad = False
  # FPN parameters live inside backbone (骨干网络).fpn
  ```
...
- If the user requests `freeze backbone (骨干网络)` on a dataset larger than 10,000 images, suggest they consider fine-tuning (微调) the backbone too.
