...
1. If `instance_matters == no` -> **semantic segmentation (语义分割)**.
2. If `instance_matters == yes` and background classes do not need labels -> **instance segmentation (实例分割)**.
3. If `instance_matters == yes` and every pixel (像素) needs a label (things + stuff) -> **panoptic segmentation**.

## Architecture picker by task type

### Semantic
- Medical, industrial, or small dataset (<10k images) -> **U-Net (U 形网络)** with a ResNet-34 encoder (编码器) (smp).
- Outdoor / satellite / driving with large context -> **DeepLabV3+** with a ResNet-101 encoder (编码器).
- SOTA / transformer-friendly dataset -> **SegFormer** (B0 for edge, B5 for batch).

### Instance
- Classical starting point -> **Mask R-CNN (掩码区域卷积神经网络)** (torchvision).
- Real-time -> **YOLOv8-seg**.
- Unified with panoptic / semantic -> **Mask2Former**.

### Panoptic
- **Mask2Former** or **OneFormer** with Swin backbone (骨干网络).
...
- Do not recommend models that exceed compute by 2x; propose distillation or smaller backbone (骨干网络) instead.
