...
description: Write a clean evaluation harness for recall (召回率)@K with train/val/gallery splits and proper data contract
version: 1.0.0
phase: 4
lesson: 20
tags: [retrieval, evaluation, recall (召回率), faiss]
---

# Recall@K 运行ner

Turn a folder of query and gallery images plus labels into a reproducible recall (召回率)@K number.

## When to use

- First retrieval benchmark for a new backbone (骨干网络).
- Tracking embedding (嵌入) quality across fine-tune (微调) epochs.
...
- `encoder_fn`: callable `image (图像) -> embedding (嵌入)` (precomputed or live).
- `ks`: list like `[1, 5, 10]`.

## Steps

1. Encode every gallery image (图像) once. Save as numpy array.
2. Encode every query image (图像).
...
7. Report `recall (召回率)@K = fraction of queries that had at least one correct neighbour in top K`.
...
- When a query's ground-truth label is absent from the gallery, exclude it; otherwise recall (召回率) is trivially capped below 1.
