...
- `labelled_pairs`: pair (anchor (锚框), positive) | triplet (a, p, n) | class_labels_only
...
- For `task_level == category`, strongly prefer off-the-shelf DINOv2 / CLIP (对比语言-图像预训练) before 训练 a custom loss.
- For `dataset_size < 5k`, recommend starting from a pretrained backbone (骨干网络) and 训练 only the embedding (嵌入) head to avoid overfitting (过拟合).
