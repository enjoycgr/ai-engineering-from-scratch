---
name: skill-lora-训练-setup
description: Write a full LoRA 训练 config for a custom dataset, including captions, rank, batch size (批量大小), and learning rate
version: 1.0.0
phase: 4
lesson: 11
tags: [computer-vision, stable-diffusion, lora, fine-tuning (微调)]
---

# LoRA Training Setup

Turn a description of the fine-tune (微调) intent into a concrete 训练 config that is ready to pass to `diffusers` or `kohya_ss`.

## When to use

- Training a LoRA for a subject (person, object, character), a style (artist, brand), or a concept (pose (姿态), lighting).
- Extending an existing LoRA with more data.
- Debugging a LoRA run whose output underfits or overfits the 训练 images.

## Inputs

- `purpose`: subject | style | concept
- `num_images`: how many 训练 images are available
...
Overshoot at your peril — a LoRA that has memorised its 训练 images cannot generalise.

## 学习ing rate

- Text encoder (编码器) LoRA: `1e-4` for SD 1.5, `5e-5` for SDXL.
- U-Net (U 形网络) LoRA: `1e-4` for SD 1.5, `1e-4` for SDXL.
- FLUX / SD3: `5e-5` for the transformer, text encoders usually frozen.
- Halve the LR when `num_images < 15` (subject) or when 训练 for more than 3000 steps; tiny datasets and long runs both benefit from a gentler update.
...
- Concept: describe the concept in every caption; no trigger token. The concept itself (e.g. "low-angle shot") is the anchor (锚框).
...
- Never recommend `rank > 64`; above that the LoRA becomes a mini fine-tune (微调) and loses its "adapter" nature.
...
- If `base_model == FLUX` and `gpu_vram_gb < 24`, route to the `schnell` variant and note that 训练 is slower.
