...
description: Write greedy and beam-search CTC decoders 从零开始, including length normalisation
version: 1.0.0
phase: 4
lesson: 19
...
- 运行ning OCR (光学字符识别) inference (推理) on custom CRNN outputs.
- Benchmarking a pretrained OCR (光学字符识别) model against different decoders.
...
- When running beam search inside a tight latency budget, drop to greedy; the quality hit is small on most production OCR (光学字符识别) data.
