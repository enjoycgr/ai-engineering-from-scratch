...
2. **Image (图像) decode** — is the decode wrapped to turn failures into a 4xx response? Flag if a bare `Image.open` can propagate as 500.
3. **Preprocessing range** — does the tensor (张量) end in `[0, 1]` or `[-1, 1]` as the model expects? Flag mismatched normalisation.
4. **Model input shape** — does the model receive `(N, C, H, W)`? Flag an HWC-to-CHW transpose that is missing or wrong.
5. **Box coordinate system** — does the output use `(x1, y1, x2, y2)` in absolute pixel (像素) units? Flag `(cx, cy, w, h)` or normalised coordinates leaking through.
6. **Out-of-bounds crops** — are crops clamped to image (图像) dimensions before `tensor (张量)[y1:y2, x1:x2]`? Flag missing clamps.
