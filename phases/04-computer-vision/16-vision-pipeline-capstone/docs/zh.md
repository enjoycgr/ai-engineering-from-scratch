# 构建 a Complete Vision Pipeline — Capstone
...
- Design a production vision pipeline that 检测 objects, 分类 them, and emits structured JSON — with every failure path handled
- Plug a detector (Mask R-CNN (掩码区域卷积神经网络) or YOLO (You Only Look Once)), a classifier (ConvNeXt-Tiny), and a data contract (Pydantic) into one service
- Benchmark the end-to-end pipeline and identify the first bottleneck (usually preprocessing, then the detector)
- Ship a minimal FastAPI service that accepts an image (图像) upload, runs the pipeline, and 返回 detections with classifications
...
Wiring those chains is the part that separates a ML prototype from a product. Every interface between models is a new place for bugs. Every coordinate transform, every normalisation, every mask (掩码) resize is a silent-failure candidate. A pipeline is as strong as its weakest interface.

This capstone sets up the minimum viable pipeline: detection + classification (分类) + structured output + a serving layer. Everything else in 阶段 4 slots into this skeleton: swap Mask R-CNN (掩码区域卷积神经网络) for YOLOv8, add a OCR (光学字符识别) head, add a segmentation branch, add a tracker. The architecture is stable; the pieces are pluggable.
...
When a detector 返回 boxes in `(cx, cy, w, h)` instead of `(x1, y1, x2, y2)`, Pydantic's validation fails at the boundary and you find out immediately instead of debugging a downstream crop that silently 返回 empty regions.
...
- **Out-of-bounds boxes** — clamp to image (图像) size before cropping.
- **Tiny crops** — skip classification (分类) for boxes smaller than the classifier's minimum input.
...
运行 with `uvicorn main:app --host 0.0.0.0 --port 8000`. Test with `curl -F 'file=@dog.jpg' http://localhost:8000/detect`.
...
- **Safety filters** — NSFW / PII filters run after classification (分类), before the response leaves the service.
- **Batch endpoint** — a `/detect_batch` accepting a list of image (图像) URLs for bulk processing.

For production serving, `torchserve`, `Triton Inference Server`, and `BentoML` handle batching, versioning, metrics, and health checks out of the box. 运行ning `FastAPI` directly is fine for prototypes and small-scale products.
...
1. **(Easy)** 运行 the pipeline on 10 images from any open dataset. Report the average time per stage and the distribution of detection counts per image (图像).
2. **(Medium)** Add a mask (掩码) output field to `Detection` and encode it as RLE. Verify the JSON stays under 1MB even for a 10-object image (图像).
...
|------|----------------|----------------------|
| Pipeline | "The system" | An ordered chain of preprocessing, inference (推理), and postprocessing steps with a typed interface between each pair |
...
| Postprocessing | "After the model" | NMS, mask (掩码) resize, threshold, RLE encode; cheap on GPU, expensive on CPU |
...
- [Full Stack Deep 学习ing — Deploying Models](https://fullstackdeeplearning.com/course/2022/lecture-5-deployment/) — the canonical overview of production ML deployment
