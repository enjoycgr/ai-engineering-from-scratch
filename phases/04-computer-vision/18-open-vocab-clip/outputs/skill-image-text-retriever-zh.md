...
description: 构建 an image (图像) embedding (嵌入) index with any CLIP (对比语言-图像预训练) checkpoint; support query-by-text and query-by-image
version: 1.0.0
phase: 4
lesson: 18
...
Turn a folder of images into a searchable index using CLIP (对比语言-图像预训练) embeddings.

## When to use

- 构建ing a zero-shot image (图像) search on an internal catalog.
- Deduplicating near-identical images by embedding (嵌入) distance.
- 构建ing a quick "find similar" component without a labelled dataset.

## Inputs

- `image_folder`: directory of image (图像) files.
...
1. Load the CLIP (对比语言-图像预训练) model and preprocessor.
2. Batch-encode every image (图像) in the folder. Save embeddings as (N, D) float32 + filename list.
3. 构建 a FAISS index over the embeddings. Use inner-product on L2-normalised vectors for cosine similarity.
4. Expose two query interfaces:
   - `search_by_text(text, k)` — embed the text, search.
   - `search_by_image(image_path, k)` — embed the image (图像), search.
