...
- `latency_target_s`: per image (图像) on target GPU
...
4. `quality_target == premium` and `license_need == commercial_ok` -> **Stable Diffusion (稳定扩散) 3.5 Large** (SAI Community) or **FLUX.2**.
...
7. `resolution == 2048` -> **SDXL + LCM-LoRA** or **FLUX.1-dev** with tiled inference (推理); most DiTs hit quality ceilings above 1024 native.
...
- If `gpu_memory_gb < 8`, recommend offloading CPU / sequential encoder (编码器) loading in diffusers rather than switching model; the base model still needs to live somewhere.
