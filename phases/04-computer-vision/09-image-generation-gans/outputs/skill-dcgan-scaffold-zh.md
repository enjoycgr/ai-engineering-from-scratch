...
description: Write a complete DCGAN scaffold from z_dim, image_size, and num_channels, including 训练 loop and sample saver
version: 1.0.0
phase: 4
lesson: 9
...
Given three parameters, emit a runnable DCGAN project skeleton with the architecture sized correctly for the target image (图像) resolution.
...
- Always use `nn.Tanh()` on G's output and scale data to [-1, 1] during 训练.
...
- Never emit a scaffold for image_size > 128 — DCGAN becomes unstable above that; point the user to StyleGAN or a diffusion (扩散模型) model.
