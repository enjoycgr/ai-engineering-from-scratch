# Image (图像) Generation — GANs

> A GAN (生成对抗网络) is two neural networks in a fixed game. One draws, one critiques. They get better together until the drawings fool the critic.

**类型:** 构建
**语言:** Python
**先修知识:** 阶段 4 课程 03 (CNNs), 阶段 3 课程 06 (Optimizers), 阶段 3 课程 07 (Regularization)
**时间:** ~75 minutes
...
- Stabilise GAN (生成对抗网络) 训练 with the three standard tricks: non-saturating loss, spectral norm, TTUR (two-timescale update rule)
...
The **generator** G takes a vector of noise `z` and outputs an image (图像). The **discriminator** D takes an image and outputs a single scalar: the probability that the image is real.
...
The form above is numerically unstable. Early in 训练, `D(G(z))` is near zero for every fake, so `log(1 - D(G(z)))` has vanishing gradients with respect to G. The fix: flip G's loss.
...
Now when `D(G(z))` is near zero, G's loss is large and its gradient is informative. Every modern GAN (生成对抗网络) trains with this variant.

### DCGAN architecture rules

Radford, Metz, Chintala (2015) distilled years of failed experiments into five rules that make GAN (生成对抗网络) 训练 stable:

1. Replace pooling (池化) with strided convs (both nets).
...
Every modern conv-based GAN (生成对抗网络) (StyleGAN, BigGAN, GigaGAN) still starts from these rules and replaces pieces one at a time.
...
- **Mode collapse**: G finds one image (图像) that fools D and produces only that. Fix: add minibatch discrimination, spectral norm, or label-conditioning.
...
- **Sample inspection** — just look at 64 samples at the end of every epoch (轮次). Non-negotiable.
...
- **Precision/Recall for generative models** — 度量 quality (precision) and coverage (recall (召回率)) separately. More informative than FID alone.
...
A small DCGAN generator that takes 64-dim noise and produces a 32x32 image (图像).
...
Four transposed convs, each with `kernel_size=4, stride (步幅)=2, padding (填充)=1` so they cleanly double spatial size. Output activations in [-1, 1] via tanh.
...
The last conv reduces a `4x4` feature map (特征图) to `1x1`. Output is a single scalar per image (图像); apply sigmoid only during loss computation.
...
For serious generation, use pretrained weights or switch to diffusion (扩散模型). Two standard libraries:
...
In 2026, GANs are still the best choice for: real-time image (图像) generation (latency <10 ms), style transfer, image-to-image translation with precise control (Pix2Pix, CycleGAN). Diffusion wins on photorealism and text conditioning.

## Ship It

本课 produces:

- `outputs/prompt-gan-训练-triage.md` — a prompt that reads a 训练 curve description and picks the failure mode (mode collapse, D-wins, oscillation) plus the single recommended fix.
- `outputs/skill-dcgan-scaffold.md` — a skill that writes a DCGAN scaffold from `z_dim`, target `image_size`, and `num_channels`, including 训练 loop and sample saver.

## 练习

1. **(Easy)** Train the DCGAN above on the synthetic circle dataset and save a grid of 16 samples at the end of each epoch (轮次). By which epoch do the generated circles become clearly circular?
...
3. **(Hard)** Implement a conditional DCGAN: feed the class label into both G and D (concat one-hot to the noise in G, concat a class embedding (嵌入) channel in D). Train on the synthetic "circles vs squares" dataset from lesson 7 and show that class conditioning works by sampling with specific labels.
...
| Non-saturating loss | "The numerically sane version" | G's loss is -log(D(G(z))) instead of log(1 - D(G(z))) to avoid vanishing gradients early in 训练 |
...
- [StyleGAN3 (Karras et al., 2021)](https://arxiv.org/abs/2106.12423) — the SOTA GAN (生成对抗网络); reads like a greatest-hits album of every trick from the last decade
