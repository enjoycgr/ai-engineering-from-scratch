...
description: Pick Sora 2 / 运行way Gen-5 / Wan-Video / HunyuanVideo / Cosmos for a given task, license, and latency target
phase: 4
lesson: 28
---
...
1. `interactivity == mid-rollout-steerable` -> **运行way GWM-1 Worlds** (production) or **Genie 3 research preview**.
2. `task == driving_sim` -> **NVIDIA Cosmos-Drive**.
3. `task == robotics_sim` -> **Genie Envisioner** or a latent-action-tuned **HunyuanVideo**.
4. `quality_target == premium` and `license_need == api_ok` -> **Sora 2** (best quality + synchronised audio) or **运行way Gen-5**.
...
7. default -> **运行way Gen-5** (API) for static video generation.
...
- For `task == product_ad`, prefer Sora 2 or 运行way Gen-5 for quality; open models currently trail.
...
- Never recommend generating public-use content with proprietary-data-trained models without the customer checking 训练-data licenses.
