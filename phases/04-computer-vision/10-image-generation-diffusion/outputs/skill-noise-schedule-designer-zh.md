...
phase: 4
lesson: 10
tags: [computer-vision, diffusion (扩散模型), noise-schedule, 训练]
---

# Noise Schedule Designer

A beta schedule controls how much signal is retained at each diffusion (扩散模型) step. Poor schedules cap 训练 efficiency and sample quality at every downstream decision.

## When to use

- Starting a new diffusion (扩散模型) 训练 run and picking T and beta.
- Debugging a diffusion (扩散模型) model that produces blurry samples (schedule too aggressive) or fails to learn structure (schedule too mild).
