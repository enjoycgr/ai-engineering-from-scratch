...
description: Inspect any image-shaped tensor (张量) or array and report dtype, layout, range, and whether it looks raw, normalized, or standardized
version: 1.0.0
phase: 4
lesson: 1
...
- A pretrained model 返回 garbage predictions and you suspect the preprocessing.
...
- Debugging a 训练 loop where loss is stuck at `log(num_classes)`.
...
2. **Classify rank**:
   - rank 2 -> single-channel image (图像) (H, W).
   - rank 3 -> `HWC` if the last axis is 1, 3, or 4 and is strictly smaller than the other two; otherwise `CHW`.
   - rank 4 -> prefer `NCHW` if axis 1 is in {1, 3, 4} **and** either axis 2 or axis 3 is larger than 16; otherwise prefer `NHWC`. Pure axis-1 check mis分类 small-image NHWC batches like `(3, 4, 224, 3)`.
...
6. **Recommend next action** based on the `likely target`:
   ...
   - For `训练`: standardize with dataset stats, transpose to CHW, add batch axis.
   - For `inference (推理)`: match the exact invariants in the model card.
