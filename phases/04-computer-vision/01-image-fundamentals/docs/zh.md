# Image (图像) Fundamentals — Pixels, Channels, Color Spaces

> An image (图像) is a tensor (张量) of light samples. Every vision model 你将 ever use starts from this one fact.

**类型:** 构建
**语言:** Python
**先修知识:** 阶段 1 课程 12 (Tensor (张量) Operations), 阶段 3 课程 11 (Intro to PyTorch)
**时间:** ~45 minutes
...
Every paper 你将 read, every pretrained weight 你将 download, every vision API 你将 call assumes a specific encoding of the input. Pass a `uint8` image (图像) where the model wants `float32` and it will still run — and silently produce garbage. Feed BGR to a network trained on RGB and accuracy collapses by ten points. Hand a model channels-last input when it expects channels-first and the first conv layer treats height as a feature channel. None of this throws an error. It just ruins your metrics and you spend a week hunting for a bug that lives in how you loaded the file.

A convolution (卷积) is not complicated once you know what it is sliding over. The hard part is that "an image (图像)" means different things to a camera, a JPEG decoder (解码器), PIL, OpenCV, torchvision, and a CUDA kernel (卷积核). Each stack has its own axis order, byte range, and channel convention. A vision engineer who cannot keep these straight ships broken pipelines.

本课 fixes the foundation so the rest of the phase can build on it. By the end 你将 know what a pixel (像素) is, why there are three numbers per pixel instead of one, what "normalize with ImageNet stats" actually does, and how to move between the two or three layouts that every other lesson in this phase will assume.
...
### A pixel (像素) is a sample, not a square

A camera sensor counts photons that land on a grid of tiny detectors. Each detector integrates light for a fraction of a second and emits a voltage proportional to how many photons hit it. The sensor then discretizes that voltage into an integer. One detector becomes one pixel (像素).
...
A pixel (像素) is not a coloured square with area. It is a single measurement. When you resize or rotate, you are resampling that measurement grid.

### Why three channels

One detector counts photons across the whole visible spectrum — that is grayscale. To get colour, the sensor covers the grid with a mosaic of red, green, and blue filters. After demosaicing, every spatial location has three integers: the response of the red-filtered detector, green-filtered, and blue-filtered nearby. Those three integers are a pixel (像素)'s RGB triplet.
...
Same tensor (张量), two orderings. Every library picks one.
...
CHW exists because convolution (卷积) kernels slide across H and W. Keeping the channel axis first means each kernel (卷积核) sees a contiguous 2D plane per channel, which vectorizes cleanly. Disk formats keep HWC because that matches how scanlines come out of a sensor.

The one-line conversion 你将 type a thousand times:
...
- **Grayscale** — OCR (光学字符识别), document models, any case where color is nuisance variable rather than signal.
...
- **Resize and pad** — preserves aspect ratio and every pixel (像素), adds black bars. Standard for detection and OCR (光学字符识别).
- **Resize directly to target** — stretches the image (图像). Cheap, distorts geometry, fine for many classification (分类) tasks.
...
Rule of thumb: bilinear for 训练, bicubic or lanczos for assets 你将 look at, nearest for anything containing integer class IDs.

## 构建 It

### Step 1: Load an image (图像) and inspect its shape
...
预期输出: `shape: (H, W, 3)`, `dtype: uint8`, range `[0, 255]`. That is the canonical on-disk representation whether the bytes came from a camera, a JPEG decoder (解码器), or a synthetic generator.
...
Go from raw bytes to the exact tensor (张量) a pretrained ImageNet model expects, then back.
...
Four steps, in this exact order: `Resize(256)` scales the shorter side to 256; `CenterCrop(224)` takes a 224x224 patch (图像块) from the middle; `ToTensor()` divides by 255 and swaps HWC to CHW; `Normalize` subtracts the ImageNet mean and divides by std. Reversing that order silently changes what reaches the model.
...
- `outputs/skill-image-tensor-inspector.md` — a skill that, given any image-shaped tensor (张量) or array, reports dtype, layout, range, and whether it looks raw, normalized, or standardized.

## 练习

1. **(Easy)** Load a JPEG with OpenCV (`cv2.imread`) and with Pillow. Print both shapes and the pixel (像素) at `(0, 0)`. Explain the channel-order difference, then write a one-line conversion that makes the OpenCV array identical to the Pillow one.
2. **(Medium)** Write `standardize(img, mean, std)` and its inverse that together pass a `roundtrip_max_diff <= 1` test on any uint8 image (图像). Your functions must work on a single image in HWC and on a batch in NCHW with the same call.
3. **(Hard)** Take a 3-channel ImageNet-standardized tensor (张量) and run it through a 1x1 conv that learns a weighted mixture of RGB into a single grayscale channel. Initialize the weights to `[0.299, 0.587, 0.114]`, freeze them, and verify the output matches your manual `rgb_to_grayscale` to within floating-point error. What other classical color-space transforms can be written as 1x1 convolutions?

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| Pixel (像素) | "A coloured square" | One sample of light intensity at one grid location — three numbers for colour, one for grayscale |
| Channel | "The colour" | One of the parallel spatial grids stacked into an image (图像) tensor (张量); last axis in HWC, first in CHW |
| HWC / CHW | "The shape" | Axis orderings for an image (图像) tensor (张量); disk and PIL use HWC, PyTorch and cuDNN use CHW |
| Normalize | "Scale the image (图像)" | Divide by 255 so pixels live in [0, 1] — necessary but not sufficient |
| Standardize | "Zero-center" | Subtract mean and divide by std per channel so the input distribution matches what the model was trained on |
| Grayscale conversion | "Average the channels" | A weighted sum with coefficients 0.299/0.587/0.114 that matches human luminance perception |
| Interpolation | "How resize picks pixels" | The rule that decides output values when the new grid does not align with the old one — nearest for labels, bilinear for 训练, bicubic for display |
...
- [PyTorch Vision Transforms Docs](https://pytorch.org/vision/stable/transforms.html) — the full pipeline of transforms 你将 actually compose in production
