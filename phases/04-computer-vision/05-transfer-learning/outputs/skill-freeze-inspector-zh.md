...
- Before the first 训练 step of a fine-tune (微调) run.
- After calling `freeze_bn_stats` or any helper that flips BN mode.
- When val accuracy is stuck at random and you suspect nothing is actually 训练.

## Inputs

- `model`: a PyTorch `nn.Module`.
- `optimizer`: the optimizer about to be used for 训练.
