...
description: Pick feature extraction vs progressive vs end-to-end fine-tuning (微调) given dataset size, domain distance, and compute budget
phase: 4
lesson: 5
---
...
- `task_type`: classification (分类) | detection | segmentation | embedding (嵌入)
...
2. `1,000 <= num_train_labels < 10,000` and `domain_distance == close` -> `partial_fine_tune` (freeze stem + stage 1, fine-tune (微调) rest).
3. `1,000 <= num_train_labels < 10,000` and `domain_distance in [medium, far]` -> `partial_fine_tune` with the stem frozen only; unfreeze the FPN/decoder (解码器) and top stages.
...
7. `compute_budget == edge` -> distil the result; never ship a 100M+ param backbone (骨干网络) to edge regardless of regime.
...
- Always report both `linear_probe_val_acc` and final `fine_tune_val_acc`. If the fine-tune (微调) ends below the probe, the plan is wrong.
...
- Never recommend fine-tuning (微调) every layer at the same LR unless the user explicitly asks for it.
