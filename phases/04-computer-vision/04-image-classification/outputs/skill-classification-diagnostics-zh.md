...
phase: 4
lesson: 4
tags: [computer-vision, classification (分类), evaluation, debugging]
---
...
- Between 训练 runs to decide what to change next.
...
- Optional `class_priors`: per-class 训练 frequency (defaults to the row sums of `cm`).
...
4. **Classify the failure mode** for each worst class. Use these quantitative thresholds so the label is reproducible:
   ...
   - `imbalance` — the class has `< 0.5x` the 训练 count of its top confuser.
...
5. **Recommend the single most impactful next action**:
   ...
   - `systematic` -> increase data for the class or fine-tune (微调) with a higher weight on this class's loss.
...
- When precision and recall (召回率) disagree by more than 0.2, always flag label noise as a candidate — real classes usually have aligned P and R after 训练.
