# Image (图像) Classification

> A classifier is a function from pixels to a probability distribution over classes. Everything else is plumbing.

**类型:** 构建
**语言:** Python
**先修知识:** 阶段 2 课程 09 (Model Evaluation), 阶段 3 课程 10 (Mini Framework), 阶段 4 课程 03 (CNNs)
**时间:** ~75 minutes

## 学习目标

- 构建 an end-to-end image (图像) classification (分类) pipeline on CIFAR-10: dataset, augmentation, model, 训练 loop, evaluation
...
- Implement mixup, cutout, and label smoothing 从零开始 and justify when each is worth adding
- Read a confusion matrix and a per-class precision/recall (召回率) table to diagnose dataset and model failures beyond aggregate accuracy

## The Problem

Every vision task that ships reduces to image (图像) classification (分类) at some level. Detection 分类 regions. Segmentation 分类 pixels. Retrieval ranks by similarity to class centroids. Getting classification right — the dataset loop, the augmentation policy, the loss, the evaluation — is the skill that transfers to every other task in the phase.

Most classification (分类) bugs are not in the model. They live in the pipeline: a broken normalisation, an unshuffled 训练 set, augmentation that distorts labels, a validation split contaminated by 训练 data, a learning rate that silently diverges after epoch (轮次) 30. A CNN (卷积神经网络) that would hit 93% on CIFAR-10 with a correct setup commonly scores 70-75% with a broken one, and the loss curve looks plausible the whole time.

本课 wires the entire pipeline 手写 so every part is inspectable. 你将 not use anything from `torchvision.datasets` that could hide a bug.

## The Concept

### The classification (分类) pipeline
...
The right-hand form is the numerically stable one (log-sum-exp). PyTorch's `nn.CrossEntropyLoss` fuses softmax (软最大值) + NLL in one op and takes raw logits directly. Applying softmax yourself first is almost always a bug — you compute log(softmax(softmax(z))), a meaningless quantity.

### Why augmentation works

A CNN (卷积神经网络) has inductive bias for translation (from weight sharing) but no built-in invariance to crops, flips, colour jitter, or occlusion. The only way to teach it those invariances is to show it pixels that exercise them. Every random transform during 训练 is a way of saying: "these two images have the same label; learn the features that ignore the difference."
...
A cousin of mixup. Instead of 训练 against `[0, 0, 1, 0, 0]`, train against `[eps/C, eps/C, 1-eps, eps/C, eps/C]` for a small `eps` like 0.1. Stops the model from producing arbitrarily sharp logits and improves calibration at almost no cost. Built into `nn.CrossEntropyLoss(label_smoothing=0.1)` since PyTorch 1.10.

### Evaluation beyond accuracy

Aggregate accuracy hides imbalance. A 90-10 binary classifier that always 预测 the majority class scores 90%. The tools that actually tell you what is happening:
...
CIFAR-10 lives on disk. To make 本课 reproducible and fast we build a synthetic dataset that looks like CIFAR — 32x32 RGB images with class-specific structure the model must learn. The exact same pipeline works unchanged on real CIFAR-10.
...
Mixes two images and two labels inside the 训练 step. Implemented as a batch transform so it lives next to the forward pass rather than inside the dataset.
...
The complete recipe: one pass over the data, gradients once per batch, scheduler stepped once per epoch (轮次).
...
1. `model.train()` before 训练, `model.eval()` before evaluation — flips dropout (随机失活) and batchnorm behaviour.
...
5. Argmax against raw logits, not softmax (软最大值) — same result, one fewer op.
...
Two things to notice: the mean/std are **dataset-specific** — computed on the CIFAR-10 训练 set, not ImageNet — and the reflect pad is the community-default crop policy. Copy-pasting ImageNet stats here is a ~1% accuracy leak that nobody catches until someone profiles the model.

## Ship It

本课 produces:

- `outputs/prompt-classifier-pipeline-auditor.md` — a prompt that audits a 训练 script for the five invariants above and surfaces the first violation.
...
2. **(Medium)** Implement Cutout — zero out a random 8x8 square in each 训练 image (图像) — and run an ablation vs no augmentation, hflip+crop, hflip+crop+cutout, hflip+crop+mixup. Report val accuracy for each.
3. **(Hard)** 构建 a CIFAR-100 pipeline (100 classes, same input size) and reproduce a ResNet-34 训练 run to within 1% of published accuracy. Extras: sweep three learning rates and two weight decays, log to a local CSV, produce the final confusion-matrix-top-confusions table.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| Logits | "Raw outputs" | The pre-softmax vector of C numbers per image (图像); cross-entropy expects these, not softmaxed values |
| Cross-entropy | "The loss" | Negative log-probability of the correct class; combines log-softmax and NLL in one stable op |
| DataLoader | "The batcher" | Wraps a dataset with shuffling, batching, and (optional) multi-worker loading; gets blamed for half of 训练 bugs |
| Augmentation | "Random transforms" | Any pixel-level transform at 训练 time that preserves the label; teaches invariances the CNN (卷积神经网络) does not have natively |
...
- [CS231n: Training Neural Networks](https://cs231n.github.io/neural-networks-3/) — still the clearest tour of the 训练 pipeline at a single page
- [Bag of Tricks for Image (图像) Classification (He et al., 2019)](https://arxiv.org/abs/1812.01187) — every small trick that together adds 3-4% to ResNet (残差网络) accuracy on ImageNet
