...
description: Instrument a production VLM (视觉语言模型) endpoint with Cross-Modal Error Rate monitoring, dashboards, and alerts
version: 1.0.0
phase: 4
lesson: 25
...
- Deploying any VLM (视觉语言模型) endpoint that produces text grounded on images.
...
- `text_confidence`: mean per-token probability after softmax (软最大值), in `[0, 1]`. Compute as `exp(mean(log_probs))`. Do not pass raw logits; raw logits are unbounded and `conf_threshold` assumes a probability.
- `image_embedding`: CLIP-family embedding (嵌入) of the image (图像) (DINOv3, SigLIP, CLIP (对比语言-图像预训练)).
- `text_embedding`: CLIP-family embedding (嵌入) of the generated text.
...
Embeddings are 1-D PyTorch tensors (`torch.float32`) from an independent CLIP-family encoder (编码器). If you use NumPy arrays, swap `.norm()` for `np.linalg.norm(...)` and cast the output accordingly.
...
3. Check the input distribution (new file format? new image (图像) source? compressed differently?).
4. Route the affected traffic to human review until the spike resolves.
5. If the spike is persistent, fine-tune (微调) or replace the model; do not suppress the alert.

## Rules

- Never compute CMER using the VLM (视觉语言模型)'s own embeddings; use an independent encoder (编码器) (DINOv3, SigLIP, or CLIP-L/14). Otherwise you are measuring the model's self-consistency, not alignment.
...
- Do not ship a VLM (视觉语言模型) endpoint without CMER monitoring; hallucinations are the dominant production failure mode and silent without this metric.
