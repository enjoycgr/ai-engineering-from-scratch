...
description: Write the complete linear-probe evaluation for any frozen encoder (编码器) and labelled dataset
version: 1.0.0
phase: 4
lesson: 17
tags: [self-supervised (自监督), evaluation, linear-probe, pytorch]
---

# Linear Probe 运行ner

Evaluate a frozen encoder (编码器)'s features by 训练 a single linear classifier on top. The standard evaluation for every self-supervised (自监督) paper.

## When to use

- Comparing self-supervised (自监督) checkpoints.
- Tracking feature quality over pre训练 epochs.
- Deciding whether a pretrained encoder (编码器) is good enough for a downstream task without fine-tuning (微调).

## Inputs

- `encoder (编码器)`: frozen `nn.Module` returning a fixed-dim feature per image (图像).
- `feature_dim`: dimensionality of the encoder (编码器) output.
- `train_dataset`: labelled dataset (image (图像), class_id).
...
1. Set encoder (编码器) to eval mode and `requires_grad=False` on every parameter.
...
5. Report top-1 accuracy on val at the end of 训练.
...
- Never update encoder (编码器) weights during linear probe; that would be a fine-tune (微调), not a probe.
- Precompute features once; re训练 the encoder (编码器) on every epoch (轮次) wastes 100x compute.
- Use SGD with cosine schedule and no weight decay; Adam sometimes underperforms here.
- Sweep learning rates at least once per encoder (编码器) family; the optimum varies across SSL methods.
