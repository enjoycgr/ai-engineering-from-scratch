...
- Starting a new point-cloud classification (分类) / segmentation project.
...
- Never augment during evaluation; only during 训练.
- If point cloud (点云) files include colour or normals as extra channels, extend the Dataset to return a `(3 + C, num_points)` tensor (张量), not just xyz.
