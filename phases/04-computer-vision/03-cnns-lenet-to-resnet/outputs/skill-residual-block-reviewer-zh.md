...
description: Review a PyTorch residual (残差) block for skip-connection correctness, BN placement, activation order, and shape alignment
version: 1.0.0
phase: 4
lesson: 3
...
A focused reviewer for any PyTorch `nn.Module` claiming to implement a residual (残差) block. Catches the four mistakes that account for almost every broken ResNet (残差网络) rewrite.
...
- You are reviewing a PR that changes ResNet (残差网络) internals (pre-activation, squeeze-excite, anti-alias).
...
For any block with `stride (步幅) != 1` or `in_channels != out_channels`, the shortcut path **must** be a shape-matching module — typically a 1x1 conv plus BN. A bare `nn.Identity()` in this case is a guaranteed shape-mismatch error at forward time.
...
The addition `out + shortcut(x)` must happen **before** the final ReLU (post-activation, original ResNet (残差网络)) or the final ReLU must be absent entirely (pre-activation ResNet v2). A block that applies ReLU in the main branch and then adds a raw shortcut produces an asymmetric activation range that hurts 训练.
...
`nn.ReLU(inplace=True)` on the tensor (张量) that will be added to the shortcut overwrites values that may still be needed for the residual (残差) add. Flag any `inplace=True` that is not followed by a layer that produces a new tensor before the add.
