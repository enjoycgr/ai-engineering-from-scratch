...
- never rely on single-image 3D lifting when absolute depth (深度) is required; it 预测 only relative pose (姿态).
...
### Custom object pose (姿态)

- `dimension == 2D` -> train an HRNet-style heatmap (热力图) head on your dataset; 500+ annotated images minimum.
