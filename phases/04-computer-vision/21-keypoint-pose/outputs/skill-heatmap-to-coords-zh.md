...
description: Write the sub-pixel heatmap-to-coordinate routine used by every production pose (姿态) model
version: 1.0.0
phase: 4
lesson: 21
tags: [keypoint (关键点), pose (姿态), subpixel, inference (推理)]
---

# Heatmap to Coords

Turn raw keypoint (关键点) heatmaps into sub-pixel precise coordinates. The cheapest accuracy upgrade in every pose (姿态) pipeline.

## When to use

- Deploying a heatmap-based keypoint (关键点) model.
- Benchmarking pose (姿态) metrics — OKS is extremely sensitive to sub-pixel accuracy.
- Porting pose (姿态) code from one framework to another.

## Inputs

- `heatmaps`: `(N, K, H, W)` tensor (张量), per-keypoint heatmaps from the model.
- `confidence_threshold`: discard keypoints whose peak is below this value.

## Steps

1. **Argmax** each heatmap (热力图) to find the integer peak location.
...
4. **Confidence** — return the peak value per keypoint (关键点); clients 用它来 mask (掩码) low-confidence predictions.
5. **Boundary case** — when the peak lands on the first or last pixel (像素) along an axis, one of the neighbours is clamped; the offset collapses to zero, which is the safest fallback.
...
- Return confidence alongside coordinates so clients can mask (掩码) low-confidence points.
- Sub-pixel refinement only helps when the heatmap (热力图) is smooth around the peak — check that 训练 used a Gaussian target with sigma >= 1.
- For very small heatmap (热力图) resolutions (< 48x48), consider upsampling the heatmap to full image (图像) size before extracting coordinates; the sub-pixel offset scales with the stride (步幅).
