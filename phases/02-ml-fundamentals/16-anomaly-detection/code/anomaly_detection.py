...
def zscore_detect(X, threshold=3.0):
    """Z-score 异常检测：标记任何特征超过 threshold 个标准差的点。"""
    mean = X.mean(axis=0)
    std = X.std(axis=0)
    std[std == 0] = 1.0
    z = np.abs((X - mean) / std)
...
def _c_factor(n):
    """Isolation Forest 的归一化常数：n 个样本的期望路径长度。"""
    if n <= 1:
        return 0.0
    if n == 2:
        return 1.0
...
def demo_comparison():
    """并排比较 Z-score、IQR 和 Isolation Forest。"""
