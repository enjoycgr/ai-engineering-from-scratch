"""完整 Transformer：纯标准库实现的编码器 + 解码器块。

演示内容：
- LayerNorm vs RMSNorm
- ReLU-FFN vs SwiGLU FFN
- 编码器块（双向）vs 解码器块（因果 + 交叉注意力）
- pre-norm 接线方式（2026 年默认）
"""
...
def main():
    ...
    print()
    print("encoder output 前 3 个单元格:")
    for i in range(3):
        print("  " + "  ".join(f"{v:+.3f}" for v in enc_out.row(i)[:4]))
    print()
    print("decoder output 前 3 个单元格:")
    for i in range(3):
        print("  " + "  ".join(f"{v:+.3f}" for v in dec_out.row(i)[:4]))
    print()
    print("stack: 2 层编码器 + 2 层解码器, pre-norm, RMSNorm, SwiGLU.")
    print("这是 2026 年的块骨架（不含 RoPE）。")
