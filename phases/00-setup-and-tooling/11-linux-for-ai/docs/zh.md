...
> 大多数 AI 运行在 Linux 上。你需要了解足够多的知识以免陷入困境。
...
- 使用 `apt` 安装系统包，为 AI 工作配置一台新的 GPU 机器
- 识别在远程机器上工作时容易让开发者踩坑的 macOS 与 Linux 差异

## 问题所在

你在 macOS 或 Windows 上开发。但当你 SSH 到云 GPU 服务器、租用 Lambda 实例或启动 EC2 机器的那一刻，你面对的是 Ubuntu。终端是你唯一的界面。没有 Finder，没有 Explorer，没有 GUI。如果你无法从命令行导航文件系统、安装包和管理进程，你就会在闲置的 GPU 时间里花钱去搜索 "how to unzip a file in Linux"。

这是一份生存指南。它恰好涵盖了你在远程 Linux 机器上进行 AI 工作所需的一切。不多不少。
...
Linux 中的每个文件都有所有者和权限位。当脚本无法执行或你无法写入某个目录时会遇到这个问题。
...
当出现 "Permission denied" 时，几乎都是权限问题。`chmod +x` 或 `sudo` 能解决大多数情况。
...
apt list --installed        # 已安装什么？
sudo apt remove htop        # 卸载
```

在全新 GPU 服务器上你会安装的常见包：
...
在云 GPU 实例上，你通常是唯一用户且已有 sudo 权限。不要所有操作都用 root。仅在需要时使用 sudo。
...
systemd 管理服务（后台守护进程）。如果你运行 inference (推理) 服务器时会用到它：
...
对于大文件传输，使用 `rsync` 而不是 `scp`。它只传输变化的字节，并能处理中断的连接。

## tmux：保持会话存活

当你 SSH 到远程服务器时，合上笔记本电脑会终止你的训练运行。tmux 防止这种情况。
...
始终在 tmux 内运行长时间训练作业。始终如此。

## Windows 用户使用 WSL2

如果你使用 Windows，WSL2 提供真正的 Linux 环境，无需双系统启动。
...
WSL2 运行真正的 Linux 内核。本课中的所有内容都可以在其中运行。你的 Windows 文件在 WSL 内位于 `/mnt/c/Users/YourName/`。

在 Windows 端安装 NVIDIA 驱动即可实现 GPU 直通。安装 Windows 版 NVIDIA 驱动（不是 Linux 版），CUDA 将在 WSL2 内可用。

## 陷阱：macOS 到 Linux

如果你从 macOS 转来，这些会让你踩坑：

| macOS | Linux | 注意事项 |
|-------|-------|-------|
| `brew install` | `sudo apt install` | 包名有时不同。`brew install htop` vs `sudo apt install htop` 没问题，但 `brew install readline` vs `sudo apt install libreadline-dev` 不同。 |
| `open file.txt` | `xdg-open file.txt` | 但在远程服务器上没有 GUI。使用 `cat` 或 `less`。 |
...
| `sed -i '' 's/a/b/' file` | `sed -i 's/a/b/' file` | macOS sed 在 `-i` 后需要一个空字符串参数。Linux 不需要。 |
...
1. SSH 到任何 Linux 机器（或打开 WSL2）并导航到你的主目录。创建一个项目文件夹，用 `touch` 在其中创建三个空文件，然后用 `ls -la` 列出它们。
2. 用 apt 安装 `htop`，运行它，找出哪个进程使用最多内存。
3. 启动一个 tmux 会话，在其中运行 `sleep 300`，分离，列出会话，然后重新挂载。
4. 使用 `df -h` 检查可用磁盘空间，然后使用 `du -sh ~/.cache/*` 找出缓存中占用空间的内容。
5. 使用 `scp` 将文件从本地机器传输到远程机器，然后用 `rsync` 执行相同传输并比较体验。
